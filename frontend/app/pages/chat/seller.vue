...
const loadError = ref("");
const sessions = ref<SellerChatSessionItem[]>([]);
...
async function fetchSellerSessions() {
  if (!user.value) {
  ...
  try {
    const res = await api<ApiResponse<SellerChatSessionItem[]>>(
      "/api/chat/sessions/seller",
      {
        method: "GET",
        query: {
    ...
    if (res.code !== 200) {
      throw new Error(res.message || "获取收到的咨询失败");
    }
    ...
    sessions.value = [];
    loadError.value =
      error instanceof Error ? error.message : "获取收到的咨询失败";
  } finally {
    loading.value = false;
  }
}
  ...
  <div class="space-y-6">
    <div class="flex items-center justify-between gap-4">
      <div>
        ...
        <p class="mt-2 text-sm text-muted">查看你作为卖家收到的聊天会话</p>
      </div>
    ...
    <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
      ...
      <UCard class="rounded-2xl shadow-sm">
        ...
        <div class="mt-2 text-2xl font-bold">卖家</div>
      </UCard>
    </div>
            ...
            >
              <div class="min-w-0">
                <div class="flex flex-wrap items-center gap-2">
                  ...
                  <UBadge color="success" variant="soft">卖家收到</UBadge>
                </div>
                ...
                <p class="mt-1 text-sm text-muted">
                  买家：
                  {{ session.buyerUsername || `用户 ${session.buyerId}` }}
                </p>
