<script setup lang="ts">
const { user } = useAuth();
const {
  buyerSessions,
  buyerUnreadSessions,
  refreshing,
  loadError,
  refresh,
  buyerUnreadSessionCount,
  buyerTotalUnreadMessageCount,
} = useChatUnread();

const currentUserId = computed(() => user.value?.id ?? 0);
const totalSessionCount = computed(() => buyerSessions.value.length);

onMounted(() => {
  void refresh();
});
</script>

<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between gap-4">
      <div>
        <h1 class="text-2xl font-bold">我发起的</h1>
        <p class="mt-2 text-sm text-muted">
          查看你作为买家发起的会话，以及卖家还有哪些回复未读
        </p>
      </div>

      <UButton
        icon="i-lucide-refresh-cw"
        variant="soft"
        :loading="refreshing"
        @click="refresh"
      >
        刷新
      </UButton>
    </div>

    <div
      v-if="loadError && !buyerSessions.length"
      class="rounded-xl border border-red-200 bg-red-50 px-4 py-10 text-center"
    >
      <p class="text-sm text-red-600">{{ loadError }}</p>
    </div>

    <template v-else>
      <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
        <UCard class="rounded-2xl shadow-sm">
          <div class="text-sm text-muted">会话总数</div>
          <div class="mt-2 text-2xl font-bold">{{ totalSessionCount }}</div>
        </UCard>

        <UCard class="rounded-2xl shadow-sm">
          <div class="text-sm text-muted">未读会话数</div>
          <div class="mt-2 text-2xl font-bold">
            {{ buyerUnreadSessionCount }}
          </div>
        </UCard>

        <UCard class="rounded-2xl shadow-sm">
          <div class="text-sm text-muted">未读消息数</div>
          <div class="mt-2 text-2xl font-bold">
            {{ buyerTotalUnreadMessageCount }}
          </div>
        </UCard>
      </div>

      <ChatSessionSection
        title="未读会话"
        description="这些会话里有卖家发来的新消息。"
        :sessions="buyerUnreadSessions"
        :current-user-id="currentUserId"
        empty-text="当前没有未读会话"
      />

      <ChatSessionSection
        title="会话列表"
        description="这里只展示你作为买家发起的会话。"
        :sessions="buyerSessions"
        :current-user-id="currentUserId"
        empty-text="你还没有发起过聊天会话"
      />
    </template>
  </div>
</template>
